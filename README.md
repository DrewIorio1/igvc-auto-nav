...
-	├── CostMapPlanner.py           # A controller for detecting non drivable areas in images using YOLOP.
-	├── DrivableAreaController.py   # A controller for detecting drivable areas in images using YOLOPv2.
-	├── GlobalNavController.py      # Testing for Global Navigatoin Controller
-	├── RRT.py                      # Path planning Sample Code with Randomized Rapidly-Exploring Random Trees (RRT)
-	├── RRTPathToGPSConverter.py    # A module to convert RRT paths from local coordinates to GPS coordinates.
-	├── RRTStar.py                  # Path planning Sample Code with Randomized Rapidly-Exploring Random Trees for dynamic envoriments
-	├── RobotLocalization.py        # A controller for estimating the robot's pose using odometry and optional IMU data.
-	├── Utility.py                  # Conains supporting functions for the fusion_node
-	├── WaypointNavigator.py        # Calculate and navigate through waypoints using GPS coordinates.
-	├── __init__.py
-	├── best.pt
-	├── fusion_node.py              # Fused Controller for Robot Navigation
-	├── robot_controller.py         # Handles both coordnate and gps for speed navigaton
-	├── waypoints.yaml
-	├── waypoints_prod.yaml
-	├── waypoints_test.yaml
-	├── yolop_pretrained.pt
